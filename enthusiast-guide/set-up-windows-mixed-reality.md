...
如果你只计划使用 Xbox 游戏板或键盘和鼠标，则无需配对控制器。  如果你计划使用控制器，则应将其配对。
...
**选项2：设置 " (" （也称为 "桌面"）)** 体验在你的办公桌上工作。 如果空间不够大，则这是一个不错的选择。 这也意味着你将在没有边界的情况下使用耳机。 你需要保留在一个位置，因为你没有边界来帮助你避免物理障碍。 某些应用和游戏不是作为边界体验设计的，因此它们可能无法按预期工作。
...
Windows Mixed Reality 中支持的最大边界大小为 18x18ft (5.7 x 5.7 m) 或 13 ft (4 m) radius 从中心开始。 边界大小取决于定位点和定位点之间的距离，你可以在风险的稳定性之前进行移动。  Windows Mixed Reality 建立在一个阶段抽象之上，这一阶段就是您在其中移动的空间。 这一阶段依赖于单个定位点，几乎每个应用程序也会假设– Naopak 和 Oculus 如何处理其单一坐标系统。  这一点很重要，因为在进行内部的跟踪时，如果您从锚点中进一步移走，则耳机跟踪是可靠的，使边界保持稳定。  如果边界旨在帮助避免出现物理障碍，就会出现更多的问题。  两个因素决定了最大边界大小。 Windows Mixed Reality 耳机可提供最佳房间缩放体验的最大距离（对于大多数 Windows Mixed Reality 耳机，最大大小为10英尺 (3 m) 。
...
除非购买了包含集成 AKG 耳机和双麦克风阵列的 Samsung HMD 太空，否则，需要使用麦克风和耳机获取音频耳机，并将其插入耳机的3.5 毫米音频插孔。 用于耳机的3.5 毫米音频插孔位于耳机面板的底部，或连接到耳机面板的短音频电缆的末端，具体取决于耳机型号。
...
* 如果以下情况，请使用 **自动增加 (最佳性能)** 选项：
    * 你的电脑不满足你的头戴显示更高分辨率显示器的最低图形硬件要求
    * 你看到的是图形性能问题
...
* **自动 (默认)**：根据电脑的硬件配置，自动选择 60 Hz 或 90-hz 体验。
* **60 Hz**
* **90 Hz**
