...
* **无扫描体验**
   ...
   * 在这种情况下，空间图面观察程序边界卷应更新为正文锁定的空间 [坐标系](coordinate-systems.md)，以便它们跟随用户。

* **查找合适的位置**
   * 应用程序可能设计为在具有特定要求的位置使用。
   * 例如，应用程序可能需要用户周围的空白区域，以便他们可以安全地练习全息影像。
   * 应用程序应会向用户传达任何特定的要求，并借助明确的视觉反馈来强化这些要求。
   * 此示例中，应用程序应可视化所需空白区域的范围，并直观地突出显示此区域中是否存在任何不需要的对象。
   * 在这种情况下，空间图面观察器边界卷应在所选位置使用世界锁定的空间坐标系。 [](coordinate-systems.md)

* **查找合适的图面配置**
   * 应用程序可能需要特定的表面配置，例如两个大型的平墙，以创建一个镜像全息门面。
   * 在这种情况下，应用程序需要分析空间映射提供的表面以检测合适的表面，并引导用户指向它们。
   * 如果应用程序的图面分析不可靠，用户应具有回退选项。 例如，如果应用程序错误地将门道标识为平墙，则用户需要一种简单的方式来更正此错误。

* **扫描部分环境**
   * 应用程序可能只希望按用户指示捕获环境的一部分。
   * 例如，应用程序会扫描房间的一部分，以便用户可以发布全息分类广告，寻找想要销售的餐厅。
   * 在这种情况下，应用程序应捕获用户在扫描期间观察到的区域的空间映射数据。

* **扫描整个房间**
   * 应用程序可能需要扫描当前房间的所有表面，包括用户后面的表面。
...
* **指导用户避免空间映射数据中的错误**
   * 应用程序可能需要在用户扫描其环境时为用户提供指导。
   * 这可以帮助用户避免空间映射数据中的某些类型的错误[](spatial-mapping.md#what-influences-spatial-mapping-quality)，例如，避免使用日照窗口或镜像。

要注意的一个额外详细信息是空间映射数据的"范围"不是无限的。 虽然空间映射确实会构建一个包含大型空间的永久数据库，但它仅使数据可供用户周围大小有限的"气泡"中的应用程序使用。 如果从长云的开头开始，距离起点足够远，则最终，起点处的空间图面将消失。 可以在这些图面从可用空间映射数据中消失后，在应用程序中缓存这些图面，从而缓解这种情况。

## <a name="mesh-processing"></a>网格处理

它可以帮助检测图面中的常见错误类型，并根据需要筛选、删除或修改空间映射数据。

请记住，空间映射数据旨在尽可能真实地应用于实际表面，因此，应用的任何处理都可能会将表面从"真实"中进一步转移。

下面是一些可能有用的不同类型的网格处理示例：

* **孔填充**
   * 如果由深色材料制造的小对象无法扫描，它将在周围的表面留下一个孔。
   * 孔会影响遮挡：全息影像可以在假定不透明的真实表面"穿过"一个孔。
   * 孔会影响光线广播：如果使用光线广播来帮助用户与表面交互，则这些射线可能不需要通过孔。 一种缓解措施是使用涵盖适当大小的区域的多个光线广播捆绑包。 这样，你可筛选"离群值"结果，以便即使一个光线广播通过小孔，聚合结果仍将有效。 但是，此方法需要计算成本。
...
* **光滑**
   ...
   * 可以平滑处理表面网格中的粗糙度。 但是，这可能会进一步将表面推离相应的实际表面。 保持紧密对应对于生成准确的全息影像遮挡以及使用户能够实现与全息图面的精确且可预测的交互非常重要。
   * 如果只需要修饰更改，则无需更改顶点位置即可平滑顶点法线。

* **平面查找**
   * 应用程序可能希望对空间映射提供的图面执行许多形式的分析。
   * 一个简单的示例是"平面查找";标识边界，主要是平面图面区域。
   * 平面区域可以用作全息工作图面，应用程序可以自动放置全息内容的区域。
   * 平面区域可以约束用户界面，以指导用户与最适合其需求的图面进行交互。
   * 平面区域可以像在现实世界中一样用于与功能对象（如LCC 屏幕、表或白板）对应的全息区域。
   * 平面区域可以定义播放区域，构成游戏级别的基础。
   * 平面区域可以通过识别实际人员可能穿过的楼层区域，帮助虚拟代理在现实世界中导航。

## <a name="prototyping-and-debugging"></a>原型制作和调试

### <a name="useful-tools"></a>有用工具

* [HoloLens 仿真](../develop/platform-capabilities-and-apis/using-the-hololens-emulator.md)器可用于开发使用空间映射的应用程序，而无需访问物理 HoloLens。 它允许你在真实的环境中模拟 HoloLens 上的实时会话，以及应用程序通常使用的所有数据，包括 HoloLens 运动、空间坐标系和空间映射网格。 这可用于提供可靠、可重复的输入，这可用于调试问题和评估对代码的更改。
* 若要重现方案，请从实时 HoloLens 捕获网络的空间映射数据，然后将数据保存到磁盘，并在稍后的调试会话中重复使用。
