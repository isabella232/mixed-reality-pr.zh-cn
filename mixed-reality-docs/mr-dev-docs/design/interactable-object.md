...
为了确保用户可以轻松地接触到所有种不可交互对象，我们建议您确保种不可交互满足最小大小 (视觉角度通常以视觉弧线度度量，) 根据它从用户的距离来度量。 视觉角度基于用户眼睛与对象之间的距离并保持不变，而目标的物理大小可能会随用户更改的距离而更改。 若要根据用户的距离确定对象的必要物理大小，请尝试使用视觉角度计算器（如 [此](https://elvers.us/perception/visualAngle/)类）。
