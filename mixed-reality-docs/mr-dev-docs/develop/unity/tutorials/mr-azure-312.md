...
<span data-ttu-id="8a76f-114">完成本课程后，你将构建一个 Windows Mixed Reality 应用程序，该应用程序将能够执行以下操作：</span><span class="sxs-lookup"><span data-stu-id="8a76f-114">After completing this course, you will have built a Windows Mixed Reality application, which will be able to do the following:</span></span>
...
## <a name="prerequisites"></a><span data-ttu-id="8a76f-131">必备条件</span><span class="sxs-lookup"><span data-stu-id="8a76f-131">Prerequisites</span></span>
...
2.  <span data-ttu-id="8a76f-202">登录后，单击左上角的 " **创建资源** "，搜索 " *Web 应用程序*"，然后单击 " **Enter**"。</span><span class="sxs-lookup"><span data-stu-id="8a76f-202">Once you are logged in, click on **Create a resource** in the top left corner, and search for *Web App bot*, and click **Enter**.</span></span>
...
5.  <span data-ttu-id="8a76f-272">尽管仍处于 **文件 > 生成设置** ，但请确保：</span><span class="sxs-lookup"><span data-stu-id="8a76f-272">While still in **File > Build Settings** and make sure that:</span></span>
    ...
    6.  <span data-ttu-id="8a76f-279">保存场景并将其添加到生成中。</span><span class="sxs-lookup"><span data-stu-id="8a76f-279">Save the scene and add it to the build.</span></span> 
...
7. <span data-ttu-id="8a76f-290">在此面板中，需要验证几项设置：</span><span class="sxs-lookup"><span data-stu-id="8a76f-290">In this panel, a few settings need to be verified:</span></span>
    ...
    2. <span data-ttu-id="8a76f-296">在 " **发布设置** " 选项卡的 " **功能**" 下，检查：</span><span class="sxs-lookup"><span data-stu-id="8a76f-296">Within the **Publishing Settings** tab, under **Capabilities**, check:</span></span>
        ...
        - <span data-ttu-id="8a76f-297">**InternetClient**</span><span class="sxs-lookup"><span data-stu-id="8a76f-297">**InternetClient**</span></span>
...
<span data-ttu-id="8a76f-364">若要创建此类：</span><span class="sxs-lookup"><span data-stu-id="8a76f-364">To create this class:</span></span> 
...
<span data-ttu-id="8a76f-384">若要创建此类：</span><span class="sxs-lookup"><span data-stu-id="8a76f-384">To create this class:</span></span> 
...
13. <span data-ttu-id="8a76f-402">要添加到此类中的最后一个方法是在场景中显示消息所必需的：</span><span class="sxs-lookup"><span data-stu-id="8a76f-402">The last method to be added to this class, is required to display the message in the scene:</span></span>
    ...
    > [!NOTE] 
    > <span data-ttu-id="8a76f-403">你可能会在 Unity 编辑器控制台中看到一个错误，缺少 **SceneOrganiser** 类。</span><span class="sxs-lookup"><span data-stu-id="8a76f-403">You may see an error within the Unity Editor Console, about missing the **SceneOrganiser** class.</span></span> <span data-ttu-id="8a76f-404">请忽略此消息，因为稍后会在本教程中创建此类。</span><span class="sxs-lookup"><span data-stu-id="8a76f-404">Disregard this message, as you will create this class later in the tutorial.</span></span>

14.  <span data-ttu-id="8a76f-405">在返回到 *Unity* 之前，请务必保存 *Visual Studio* 中所做的更改。</span><span class="sxs-lookup"><span data-stu-id="8a76f-405">Be sure to save your changes in *Visual Studio* before returning to *Unity*.</span></span>

## <a name="chapter-11--create-the-interactions-class"></a><span data-ttu-id="8a76f-406">第11章–创建交互类</span><span class="sxs-lookup"><span data-stu-id="8a76f-406">Chapter 11 – Create the Interactions class</span></span>

<span data-ttu-id="8a76f-407">现在要创建的类称为 **交互**。</span><span class="sxs-lookup"><span data-stu-id="8a76f-407">The class you are going to create now is called **Interactions**.</span></span> <span data-ttu-id="8a76f-408">此类用于检测 HoloLens 点击用户的输入。</span><span class="sxs-lookup"><span data-stu-id="8a76f-408">This class is used to detect the HoloLens Tap Input from the user.</span></span> 

<span data-ttu-id="8a76f-409">如果用户在查看场景中的 *机器人* 对象时点击，并准备好侦听语音输入，则机器人对象会将颜色更改为 **红色** ，并开始侦听语音输入。</span><span class="sxs-lookup"><span data-stu-id="8a76f-409">If the user taps while looking at the *Bot* object in the scene, and the Bot is ready to listen for voice inputs, the Bot object will change color to **red** and begin listening for voice inputs.</span></span> 

<span data-ttu-id="8a76f-410">此类继承自 **GazeInput** 类，因此，可以从该类引用 **Start ( # B1** 方法和变量，通过使用 **base** 来表示。</span><span class="sxs-lookup"><span data-stu-id="8a76f-410">This class inherits from the **GazeInput** class, and so is able to reference the **Start()** method and variables from that class, denoted by the use of **base**.</span></span> 
 
<span data-ttu-id="8a76f-411">若要创建此类：</span><span class="sxs-lookup"><span data-stu-id="8a76f-411">To create this class:</span></span>

1.  <span data-ttu-id="8a76f-412">双击 " **脚本** " 文件夹以将其打开。</span><span class="sxs-lookup"><span data-stu-id="8a76f-412">Double-click on the **Scripts** folder, to open it.</span></span> 
2.  <span data-ttu-id="8a76f-413">右键单击 " **脚本** " 文件夹中，单击 " **创建 > c # 脚本**"。</span><span class="sxs-lookup"><span data-stu-id="8a76f-413">Right-click inside the **Scripts** folder, click **Create > C# Script**.</span></span> <span data-ttu-id="8a76f-414">命名脚本 **交互**。</span><span class="sxs-lookup"><span data-stu-id="8a76f-414">Name the script **Interactions**.</span></span> 
3.  <span data-ttu-id="8a76f-415">双击新脚本以通过 Visual Studio 将其打开。</span><span class="sxs-lookup"><span data-stu-id="8a76f-415">Double-click on the new script to open it with Visual Studio.</span></span>
4.  <span data-ttu-id="8a76f-416">在 **交互** 类的顶部，将命名空间和类继承更新为与以下相同：</span><span class="sxs-lookup"><span data-stu-id="8a76f-416">Update the namespaces and the class inheritance to be the same as the following, at the top of the **Interactions** class:</span></span>
...
5.  <span data-ttu-id="8a76f-417">在 **交互** 类中添加以下变量：</span><span class="sxs-lookup"><span data-stu-id="8a76f-417">Inside the **Interactions** class add the following variable:</span></span>
    ...
    ```
6.  <span data-ttu-id="8a76f-418">然后添加 **Start ( # B1** 方法：</span><span class="sxs-lookup"><span data-stu-id="8a76f-418">Then add the **Start()** method:</span></span>
...
7.  <span data-ttu-id="8a76f-419">添加当用户在 HoloLens 相机前面执行分流手势时触发的处理程序</span><span class="sxs-lookup"><span data-stu-id="8a76f-419">Add the handler that will be triggered when the user performs the tap gesture in front of the HoloLens camera</span></span>
...
8. <span data-ttu-id="8a76f-420">在返回到 *Unity* 之前，请务必保存 *Visual Studio* 中所做的更改。</span><span class="sxs-lookup"><span data-stu-id="8a76f-420">Be sure to save your changes in *Visual Studio* before returning to *Unity*.</span></span>

## <a name="chapter-12--create-the-sceneorganiser-class"></a><span data-ttu-id="8a76f-421">第12章–创建 SceneOrganiser 类</span><span class="sxs-lookup"><span data-stu-id="8a76f-421">Chapter 12 – Create the SceneOrganiser class</span></span>

<span data-ttu-id="8a76f-422">此实验室中所需的最后一个类称为 **SceneOrganiser**。</span><span class="sxs-lookup"><span data-stu-id="8a76f-422">The last class required in this Lab is called **SceneOrganiser**.</span></span> <span data-ttu-id="8a76f-423">此类将通过将组件和脚本添加到主相机并在场景中创建相应的对象，以编程方式设置场景。</span><span class="sxs-lookup"><span data-stu-id="8a76f-423">This class will setup the scene programmatically, by adding components and scripts to the Main Camera, and creating the appropriate objects in the scene.</span></span>
 
<span data-ttu-id="8a76f-424">若要创建此类：</span><span class="sxs-lookup"><span data-stu-id="8a76f-424">To create this class:</span></span>

1.  <span data-ttu-id="8a76f-425">双击 " **脚本** " 文件夹以将其打开。</span><span class="sxs-lookup"><span data-stu-id="8a76f-425">Double-click on the **Scripts** folder, to open it.</span></span> 
2.  <span data-ttu-id="8a76f-426">右键单击 " **脚本** " 文件夹中，单击 " **创建 > c # 脚本**"。</span><span class="sxs-lookup"><span data-stu-id="8a76f-426">Right-click inside the **Scripts** folder, click **Create > C# Script**.</span></span> <span data-ttu-id="8a76f-427">将脚本命名为 **SceneOrganiser**。</span><span class="sxs-lookup"><span data-stu-id="8a76f-427">Name the script **SceneOrganiser**.</span></span> 
3.  <span data-ttu-id="8a76f-428">双击新脚本以通过 Visual Studio 将其打开。</span><span class="sxs-lookup"><span data-stu-id="8a76f-428">Double-click on the new script to open it with Visual Studio.</span></span>
4.  <span data-ttu-id="8a76f-429">在 **SceneOrganiser** 类中，添加以下变量：</span><span class="sxs-lookup"><span data-stu-id="8a76f-429">Inside the **SceneOrganiser** class add the following variables:</span></span>
...
6.  <span data-ttu-id="8a76f-430">然后添加 **唤醒 ( # B1** 并 **开始 ( # B3** 方法：</span><span class="sxs-lookup"><span data-stu-id="8a76f-430">Then add the **Awake()** and **Start()** methods:</span></span>
...
7.  <span data-ttu-id="8a76f-431">添加以下方法，负责在场景中创建机器人对象并设置参数和组件：</span><span class="sxs-lookup"><span data-stu-id="8a76f-431">Add the following method, responsible for creating the Bot object in the scene and setting up the parameters and components:</span></span>
...
7.  <span data-ttu-id="8a76f-432">添加以下方法，负责在场景中创建 UI 对象，以表示来自机器人的响应：</span><span class="sxs-lookup"><span data-stu-id="8a76f-432">Add the following method, responsible for creating the UI object in the scene, representing the responses from the Bot:</span></span>
...
8.  <span data-ttu-id="8a76f-433">在返回到 *Unity* 之前，请务必保存 *Visual Studio* 中所做的更改。</span><span class="sxs-lookup"><span data-stu-id="8a76f-433">Be sure to save your changes in *Visual Studio* before returning to *Unity*.</span></span>
9.  <span data-ttu-id="8a76f-434">在 Unity 编辑器中，将 " **SceneOrganiser** " 脚本从 "脚本" 文件夹拖到主摄像机。</span><span class="sxs-lookup"><span data-stu-id="8a76f-434">In the Unity Editor, drag the **SceneOrganiser** script from the Scripts folder to the Main Camera.</span></span> <span data-ttu-id="8a76f-435">场景 Organiser 组件现在应显示在 "照相机" 对象上，如下图所示。</span><span class="sxs-lookup"><span data-stu-id="8a76f-435">The Scene Organiser component should now appear on the Main Camera object, as shown in the image below.</span></span>

    ![创建 Azure Bot 服务](images/AzureLabs-Lab312-37.png)

## <a name="chapter-13--before-building"></a><span data-ttu-id="8a76f-437">第13章–生成之前</span><span class="sxs-lookup"><span data-stu-id="8a76f-437">Chapter 13 – Before building</span></span>

<span data-ttu-id="8a76f-438">若要对应用程序进行全面测试，需要将其旁加载到 HoloLens。</span><span class="sxs-lookup"><span data-stu-id="8a76f-438">To perform a thorough test of your application you will need to sideload it onto your HoloLens.</span></span>
<span data-ttu-id="8a76f-439">在执行此操作之前，请确保：</span><span class="sxs-lookup"><span data-stu-id="8a76f-439">Before you do, ensure that:</span></span>

-   <span data-ttu-id="8a76f-440">[**第4章**](#chapter-4--set-up-the-unity-project)中提到的所有设置都已正确设置。</span><span class="sxs-lookup"><span data-stu-id="8a76f-440">All the settings mentioned in the [**Chapter 4**](#chapter-4--set-up-the-unity-project) are set correctly.</span></span> 
-   <span data-ttu-id="8a76f-441">脚本 **SceneOrganiser** 已附加到 **摄像机主** 对象。</span><span class="sxs-lookup"><span data-stu-id="8a76f-441">The script **SceneOrganiser** is attached to the **Main Camera** object.</span></span> 
-   <span data-ttu-id="8a76f-442">在 **机器人** 类中，请确保已将 **机器人机密密钥** 插入到 **botSecret** 变量。</span><span class="sxs-lookup"><span data-stu-id="8a76f-442">In the **Bot** class, make sure you have inserted your **Bot Secret Key** into the **botSecret** variable.</span></span>

## <a name="chapter-14--build-and-sideload-to-the-hololens"></a><span data-ttu-id="8a76f-443">第14章–生成并旁加载到 HoloLens</span><span class="sxs-lookup"><span data-stu-id="8a76f-443">Chapter 14 – Build and Sideload to the HoloLens</span></span>

<span data-ttu-id="8a76f-444">此项目的 Unity 部分所需的所有内容现在均已完成，因此可以从 Unity 构建它。</span><span class="sxs-lookup"><span data-stu-id="8a76f-444">Everything needed for the Unity section of this project has now been completed, so it is time to build it from Unity.</span></span>

1.  <span data-ttu-id="8a76f-445">导航到 " **生成设置**"，" **文件 > 生成设置 ...**"。</span><span class="sxs-lookup"><span data-stu-id="8a76f-445">Navigate to **Build Settings**, **File > Build Settings…**.</span></span>
2.  <span data-ttu-id="8a76f-446">在 **生成设置** 窗口中，单击 " **生成**"。</span><span class="sxs-lookup"><span data-stu-id="8a76f-446">From the **Build Settings** window, click **Build**.</span></span>

    ![从 Unity 生成应用](images/AzureLabs-Lab312-38.png)

3.  <span data-ttu-id="8a76f-448">如果尚未这样做，请勾选 **Unity c # 项目**。</span><span class="sxs-lookup"><span data-stu-id="8a76f-448">If not already, tick **Unity C# Projects**.</span></span>
4.  <span data-ttu-id="8a76f-449">单击“生成”。</span><span class="sxs-lookup"><span data-stu-id="8a76f-449">Click **Build**.</span></span> <span data-ttu-id="8a76f-450">Unity 将启动 **文件资源管理器** 窗口，在该窗口中，需要创建一个文件夹，然后选择要在其中生成应用的文件夹。</span><span class="sxs-lookup"><span data-stu-id="8a76f-450">Unity will launch a **File Explorer** window, where you need to create and then select a folder to build the app into.</span></span> <span data-ttu-id="8a76f-451">立即创建该文件夹并将其命名为 **应用**。</span><span class="sxs-lookup"><span data-stu-id="8a76f-451">Create that folder now, and name it **App**.</span></span> <span data-ttu-id="8a76f-452">选择 **应用** 文件夹后，单击 " **选择文件夹**"。</span><span class="sxs-lookup"><span data-stu-id="8a76f-452">Then with the **App** folder selected, click **Select Folder**.</span></span> 
5.  <span data-ttu-id="8a76f-453">Unity 将开始向 **应用** 文件夹生成项目。</span><span class="sxs-lookup"><span data-stu-id="8a76f-453">Unity will begin building your project to the **App** folder.</span></span> 
6.  <span data-ttu-id="8a76f-454">Unity 完成生成后 (可能需要一些时间) ，它将在你的生成的位置上打开 " **文件资源管理器** " 窗口 (检查任务栏，因为它可能不会始终出现在 windows 上，但会通知你添加了新的窗口) 。</span><span class="sxs-lookup"><span data-stu-id="8a76f-454">Once Unity has finished building (it might take some time), it will open a **File Explorer** window at the location of your build (check your task bar, as it may not always appear above your windows, but will notify you of the addition of a new window).</span></span>

## <a name="chapter-15--deploy-to-hololens"></a><span data-ttu-id="8a76f-455">第15章–部署到 HoloLens</span><span class="sxs-lookup"><span data-stu-id="8a76f-455">Chapter 15 – Deploy to HoloLens</span></span>

<span data-ttu-id="8a76f-456">在 HoloLens 上部署：</span><span class="sxs-lookup"><span data-stu-id="8a76f-456">To deploy on HoloLens:</span></span>

1.  <span data-ttu-id="8a76f-457">需要为远程部署) 提供 HoloLens (的 IP 地址，并确保 HoloLens 处于 **开发人员模式**。</span><span class="sxs-lookup"><span data-stu-id="8a76f-457">You will need the IP Address of your HoloLens (for Remote Deploy), and to ensure your HoloLens is in **Developer Mode**.</span></span> <span data-ttu-id="8a76f-458">具体方法为：</span><span class="sxs-lookup"><span data-stu-id="8a76f-458">To do this:</span></span>

    1. <span data-ttu-id="8a76f-459">在戴上 HoloLens 的同时，请打开 **设置**。</span><span class="sxs-lookup"><span data-stu-id="8a76f-459">Whilst wearing your HoloLens, open the **Settings**.</span></span>
    2. <span data-ttu-id="8a76f-460">**Wi-Fi > 高级选项中转到网络 & Internet >**</span><span class="sxs-lookup"><span data-stu-id="8a76f-460">Go to **Network & Internet > Wi-Fi > Advanced Options**</span></span>
    3. <span data-ttu-id="8a76f-461">记下 **IPv4** 地址。</span><span class="sxs-lookup"><span data-stu-id="8a76f-461">Note the **IPv4** address.</span></span>
    4. <span data-ttu-id="8a76f-462">接下来，导航回 " **设置**"，然后为 **开发人员更新 & Security >**</span><span class="sxs-lookup"><span data-stu-id="8a76f-462">Next, navigate back to **Settings**, and then to **Update & Security > For Developers**</span></span> 
    5. <span data-ttu-id="8a76f-463">设置开发人员模式。</span><span class="sxs-lookup"><span data-stu-id="8a76f-463">Set Developer Mode On.</span></span>

2.  <span data-ttu-id="8a76f-464">导航到新的 Unity 生成 (**应用** 文件夹) 并通过 **Visual Studio** 打开解决方案文件。</span><span class="sxs-lookup"><span data-stu-id="8a76f-464">Navigate to your new Unity build (the **App** folder) and open the solution file with **Visual Studio**.</span></span>
3.  <span data-ttu-id="8a76f-465">在 **解决方案配置** 中，选择 " **调试**"。</span><span class="sxs-lookup"><span data-stu-id="8a76f-465">In the **Solution Configuration** select **Debug**.</span></span>
4.  <span data-ttu-id="8a76f-466">在 **解决方案平台** 中，选择 " **X86**， **远程计算机**"。</span><span class="sxs-lookup"><span data-stu-id="8a76f-466">In the **Solution Platform**, select **x86**, **Remote Machine**.</span></span> 

    ![从 Visual Studio 部署解决方案。](images/AzureLabs-Lab312-39.png)
 
5.  <span data-ttu-id="8a76f-468">请在 " **生成" 菜单** 中，单击 " **部署解决方案**"，将应用程序旁加载到 HoloLens。</span><span class="sxs-lookup"><span data-stu-id="8a76f-468">Go to the **Build menu** and click on **Deploy Solution**, to sideload the application to your HoloLens.</span></span>
6.  <span data-ttu-id="8a76f-469">应用现在应显示在你的 HoloLens 上已安装的应用列表中，可以启动了！</span><span class="sxs-lookup"><span data-stu-id="8a76f-469">Your app should now appear in the list of installed apps on your HoloLens, ready to be launched!</span></span>

    > [!NOTE]
    > <span data-ttu-id="8a76f-470">若要部署到沉浸式耳机，请将 " **解决方案平台** " 设置为 " *本地计算机*"，并将 **配置** 设置为 " *调试*"，将 " *x86* " 设置为 **平台**。</span><span class="sxs-lookup"><span data-stu-id="8a76f-470">To deploy to immersive headset, set the **Solution Platform** to *Local Machine*, and set the **Configuration** to *Debug*, with *x86* as the **Platform**.</span></span> <span data-ttu-id="8a76f-471">然后，使用 " **生成" 菜单** 选择 " *部署解决方案*"，将部署到本地计算机。</span><span class="sxs-lookup"><span data-stu-id="8a76f-471">Then deploy to the local machine, using the **Build menu**, selecting *Deploy Solution*.</span></span> 

## <a name="chapter-16--using-the-application-on-the-hololens"></a><span data-ttu-id="8a76f-472">第16章–在 HoloLens 上使用应用程序</span><span class="sxs-lookup"><span data-stu-id="8a76f-472">Chapter 16 – Using the application on the HoloLens</span></span>

- <span data-ttu-id="8a76f-473">启动应用程序后，会看到机器人作为你前面的蓝色球。</span><span class="sxs-lookup"><span data-stu-id="8a76f-473">Once you have launched the application, you will see the Bot as a blue sphere in front of you.</span></span>

- <span data-ttu-id="8a76f-474">在 gazing 时使用 **点击手势** 来启动会话。</span><span class="sxs-lookup"><span data-stu-id="8a76f-474">Use the **Tap Gesture** while you are gazing at the sphere to initiate a conversation.</span></span> 
 
- <span data-ttu-id="8a76f-475">等待会话开始 (UI 将在) 发生时显示一条消息。</span><span class="sxs-lookup"><span data-stu-id="8a76f-475">Wait for the conversation to start (The UI will display a message when it happens).</span></span> <span data-ttu-id="8a76f-476">收到来自机器人的介绍性消息后，再次在机器人上点击，使其变为红色并开始倾听你的声音。</span><span class="sxs-lookup"><span data-stu-id="8a76f-476">Once you receive the introductory message from the Bot, tap again on the Bot so it will turn red and begin to listen to your voice.</span></span> 

- <span data-ttu-id="8a76f-477">停止对话后，应用程序会将消息发送到机器人，并立即收到将在 UI 中显示的响应。</span><span class="sxs-lookup"><span data-stu-id="8a76f-477">Once you stop talking, your application will send your message to the Bot and you will promptly receive a response that will be displayed in the UI.</span></span> 

- <span data-ttu-id="8a76f-478">重复此过程，向机器人发送更多消息， (每次需要 sen 消息时，都必须点击) 。</span><span class="sxs-lookup"><span data-stu-id="8a76f-478">Repeat the process to send more messages to your Bot (you have to tap each time you want to sen a message).</span></span>

<span data-ttu-id="8a76f-479">此对话演示了机器人如何保留 (名称) 的信息，同时还提供了 (的已知信息，如贮存) 的项。</span><span class="sxs-lookup"><span data-stu-id="8a76f-479">This conversation demonstrates how the Bot can retain information (your name), whilst also providing known information (such as the items that are stocked).</span></span>

#### <a name="some-questions-to-ask-the-bot"></a><span data-ttu-id="8a76f-480">询问机器人的一些问题：</span><span class="sxs-lookup"><span data-stu-id="8a76f-480">Some questions to ask the Bot:</span></span>
...
## <a name="your-finished-web-app-bot-v4-application"></a><span data-ttu-id="8a76f-481">已完成的 Web 应用机器人 (v4) 应用程序</span><span class="sxs-lookup"><span data-stu-id="8a76f-481">Your finished Web App Bot (v4) application</span></span>

<span data-ttu-id="8a76f-482">恭喜，你构建了一个利用 Azure Web 应用机器人、Microsoft Bot Framework v4 的混合现实应用。</span><span class="sxs-lookup"><span data-stu-id="8a76f-482">Congratulations, you built a mixed reality app that leverages the Azure Web App Bot, Microsoft Bot Framework v4.</span></span>

![最终产品](images/AzureLabs-Lab312-00.png)

## <a name="bonus-exercises"></a><span data-ttu-id="8a76f-484">额外练习</span><span class="sxs-lookup"><span data-stu-id="8a76f-484">Bonus exercises</span></span>

### <a name="exercise-1"></a><span data-ttu-id="8a76f-485">练习1</span><span class="sxs-lookup"><span data-stu-id="8a76f-485">Exercise 1</span></span>

<span data-ttu-id="8a76f-486">此实验室中的会话结构非常基本。</span><span class="sxs-lookup"><span data-stu-id="8a76f-486">The conversation structure in this Lab is very basic.</span></span> <span data-ttu-id="8a76f-487">使用 Microsoft LUIS 为机器人指定自然语言理解功能。</span><span class="sxs-lookup"><span data-stu-id="8a76f-487">Use Microsoft LUIS to give your bot natural language understanding capabilities.</span></span>

### <a name="exercise-2"></a><span data-ttu-id="8a76f-488">练习2</span><span class="sxs-lookup"><span data-stu-id="8a76f-488">Exercise 2</span></span>

<span data-ttu-id="8a76f-489">此示例不包括终止会话和重新启动新会话。</span><span class="sxs-lookup"><span data-stu-id="8a76f-489">This example does not include terminating a conversation and restarting a new one.</span></span> <span data-ttu-id="8a76f-490">若要使机器人功能完成，请尝试将结束实现到会话中。</span><span class="sxs-lookup"><span data-stu-id="8a76f-490">To make the Bot feature complete, try to implement closure to the conversation.</span></span>
