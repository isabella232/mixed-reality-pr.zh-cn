...
> [!NOTE]
> 你使用的是 Unity 的内置旧版 XR 而不是新的 XR 插件系统，因为新系统与本系列教程中[推荐的 Unity 和 MRTK 版本](mr-learning-base-01.md#prerequisites)不完全兼容。 因此，可以忽略有关不推荐使用的内置 XR 的任何信息或警告。
...
> [!TIP]
> 可以根据需要决定是否设置“音频空间定位器”属性，如果设置，可提高项目中的音频体验。 如果将其设置为“MS HRTF 空间定位器”，则启用 Unity 的 AudioSource.spatialize 属性时将使用此空间定位器插件。 若要了解有关本主题的详细信息，请参阅[空间音频教程](unity-spatial-audio-ch1.md)。
