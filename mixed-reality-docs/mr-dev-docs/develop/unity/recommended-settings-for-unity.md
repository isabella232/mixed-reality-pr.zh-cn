...
某些应用可能不需要跟踪 (例如， [仅面向打印的应用](coordinate-systems-in-unity.md) （如360度视频查看器）) 或者可能需要在跟踪丢失时继续处理。 在这些情况下，应用可以选择不丢失跟踪行为的默认值。 选择此项的开发人员负责隐藏/禁用任何不能在跟踪丢失方案中正确呈现的对象。 在大多数情况下，建议在这种情况下呈现的内容只是正文锁定的内容，并在主相机周围居中。
