...
将 Unity 质量设置修改为"非常低"非常重要，以便应用程序在适当的帧速率下运行且性能良好，尤其是对于HoloLens开发。 对于沉浸式头戴显示设备的开发，根据为 VR 体验提供电源的桌面规格，仍可在没有最低质量参数的情况下实现帧速率。

在 Unity 2019 LTS+ 中，可以通过进入"编辑 Project 设置质量"并单击向下箭头将"默认"设置为"非常低质量"级别，来设置项目  >    >  的质量级别。

### <a name="lighting-settings"></a>照明设置

与"质量"场景设置类似，为混合现实应用程序设置最佳照明设置非常重要。 在 Unity 中，通常对场景具有最大性能影响的"照明"设置是 **"实时全局照明"。** 可以通过访问"窗口渲染照明"或"实时全局照明  >    >    >  **设置"来关闭全局照明**。

还有另一个照明设置 **，"烘焙全局照明"。** 此设置可以在沉浸式头戴显示设备上提供性能高、视觉效果突出的结果，但不适用于HoloLens开发。 **烘焙的全局** 照明仅针对静态 GameObject 进行计算，由于未知和不断变化的环境的性质HoloLens在场景中找不到这些对象。
...
1) 打开“播放器 XR 设置”（转到“编辑” > “项目设置” > “播放器” > “XR 设置”）    
2) 选中 **"在支持** 虚拟现实的 **虚拟网络中Windows Mixed Reality** SDK  >   **("启用** 深度缓冲区共享"复选框) 
...
如果使用混合[现实Toolkit标准着色器](/windows/mixed-reality/mrtk-unity/features/rendering/mrtk-standard-shader)，则呈现透明对象的深度：
...
>
> 但是，建议改为将相机的近HoloLens远平面修改为更小的范围，并保留 16 位深度格式，尤其是对于开发来说。 z 深度是沿近相机平面和远相机平面以非线性方式映射到值范围。 可以通过在场景中选择主相机，在"检查器"**下** 修改这一点，更改"近&远剪裁平面"值，以减小其 (范围，即 从 1000m 到 100m 或其他 x 值，等等) 
...
HoloLens具有移动类 CPU 和 GPU，这意味着加载应用可能需要更长的时间。 加载应用时，用户只看到黑色，因此他们可能会想知道这一点。 若要在加载过程中保护它们，可以添加全息初始屏幕。

切换全息初始屏幕：

1) 转到"**编辑**  >  **Project 设置**  >  **播放器"** 页
2) 选择 **"Windows"选项卡** 并打开"**初始图像"** 部分
3) 在"全息图像"Windows **应用>"全息初始图像"** 属性下。
    - 切换"显示 **Unity 初始屏幕"** 选项将启用或禁用 Unity 品牌初始屏幕。 如果没有 Unity Pro许可证，则始终显示 Unity 品牌初始屏幕。
    - 如果 **应用全息初始** 图像，则无论启用还是禁用"显示 Unity 初始屏幕"复选框，都会始终显示该图像。 指定自定义全息初始图像仅适用于具有 Unity Pro许可证的开发人员。
...
1) 中转到 "**编辑**"  >  **Project 设置**  >  **Player** "页
2) 选择 " **Windows 存储**" 选项卡，然后打开 "**初始图像**" 部分
3) 在 " **Windows 全息 > 跟踪丢失图像**" 属性下应用映像。
...
1) 切换到 "**编辑**  >  **Project 设置**  >  **播放器**" 页
2) 选择 **Windows 存储**"选项卡，然后打开"**初始图像**"部分
3) 修改 "**跟踪丢失时暂停" 和 "显示图像**" 复选框中 Windows 全息 >。
...
1) 中转到 "**编辑**"  >  **Project 设置**  >  **Player** "页
2) 选择 **Windows 存储**"选项卡，打开"**发布设置**"部分，然后查找"**功能**"列表
