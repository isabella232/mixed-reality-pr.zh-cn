...
ms.locfileid: "122184738"
---
# <a name="shared-experiences-in-mixed-reality"></a>混合现实中的共享体验

全息影像一个用户无需保持私密。 全息应用可能会将空间[](../../design/spatial-anchors.md)定位点从一台 HoloLens、iOS 或 Android 设备共享到另一台设备，使用户能够跨多个设备在现实世界中的同一位置渲染全息影像。
...
* **演示**：向多个用户显示相同的内容时。 例如：一位教授使用向每个人呈现的同一全息材料向多个学生进行讲座。 但是，教授可能有自己的提示和注释，而其他人可能看不到这些提示和注释。
...
**一对一** 共享体验可以提供强大的基线，理想情况下，可以在此级别创建概念证明。 但请注意，与 6 人 (大型组共享) 可能会导致技术 (数据和网络) 和社交 (与多个头像一起) 。 [](https://vimeo.com/160704056) 从小组到大型组 时，复杂性 **呈指数级增长**。
...
当共享体验可以在同一位置进行时，混合现实的优势将发挥作用。 我们称之为 **共分配**。 相反，当组分布并且至少有一个参与者不在同一物理空间中时 (与 VR 模式通常一样) 我们称之为远程 **体验**。 通常，组同时有并分配和远程参与者，例如 (会议室中的两个组) 。
...
在共分配体验之外，两个相同的实际环境的可能性很小，除非这些环境设计为相同。 你更有可能 **拥有类似的环境** 。 例如，会议室是类似的，它们通常有一个中央位置的表，周围有一个四周都由。 另一方面，房间不同，可以在无限布局数组中包括任意数目的装饰。

![表上的 Hol分](images/holograph-on-table-500px.png)

请考虑适合以下两个类别之一的共享体验：

* **类似**：环境往往具有相似的房间、环境光线和声音、物理房间大小。 例如：教授在讲座场 A 中，学生位于讲座会场 B 中。讲座场 A 的台子可能比 B 少，但他们两人可能都有一个可放置全息影像的物理台。
...
* 人们如何体验这些对象。 例如：如果你的体验对表效果最佳，而用户没有表？ 或者，在平面图面上，但用户的空间杂乱。
* 对象的规模。 例如：在表上放置六英尺人类模型可能很有挑战性，但心模型会非常成功。

### <a name="6-what-devices-are-they-using"></a>6.它们使用哪些设备？

目前，你可能经常看到两个沉浸式设备之间的共享 [**体验 (这些设备**](../../discover/immersive-headset-hardware-details.md)对于按钮和相对功能可能略有不同，但考虑到针对这些设备的解决方案，) 或两个全息设备可能略有不同。  但请考虑 **2D** (移动/桌面参与者或观察程序) 是否是必需的注意事项，尤其是在混合 **2D 和 3D** 设备的情况下。 了解参与者将使用的设备类型非常重要，不仅因为它们具有不同的保真度、数据约束和机会，还因为用户对于每个平台都有独特的期望。
...
例如，考虑Skype启动中的一HoloLens场景：用户已借助远程定位专家的帮助来修复损坏的指示灯开关。 [](https://www.youtube.com/watch?v=iBfzs3G8BEA)

![通过 Skype 修复轻型HoloLens](images/fix-a-broken-switch-with-hololens-640px.jpg)
