...
* **[布隆](../../design/system-gesture.md#bloom)/System 手势或 home**： HoloLens 布隆/系统手势或控制器的 "主页" 按钮用于返回到 shell 并执行系统操作。
...
你还可以控制模拟传感器输入的状态：
* **Reset**：这会将所有模拟传感器恢复为其默认值。  从 HoloLens 2 模拟器开始，可以通过使用相应的修改键 (s，通过使用相应的修改键 (s，将重置的范围限定为一次或两次) 使用) 或按钮 ()  (左和/或右 Alt，或游戏板上的左和/或右缓冲器) 。
* **跟踪**：循环浏览位置跟踪模式。 这包括：
