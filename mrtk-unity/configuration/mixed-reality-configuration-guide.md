...
ms.locfileid: "115214882"
---
# <a name="mrtk-profile-configuration-guide"></a>MRTK 配置文件配置指南

混合现实 Toolkit 将尽可能多的配置集中在管理工具包 (，但真正的运行时 "内容" ) 除外。

本指南是一个简单的演练，适用于当前适用于该工具包的每个配置文件屏蔽。

## <a name="the-main-mixed-reality-toolkit-configuration-profile"></a>主要混合现实 Toolkit 配置文件

主配置文件（附加到场景中的 *MixedRealityToolkit* GameObject）提供项目中 Toolkit 的主入口点。

> [!NOTE]
> 混合现实 Toolkit "锁定" 默认配置屏幕以确保你始终具有项目的一个公共起点，并且建议在项目演变时开始定义自己的设置。 在播放模式下，不能编辑 MRTK 配置。

![MRTK 配置文件](../features/images/mixed-reality-toolkit-configuration-profile-screens/MRTK_ActiveConfiguration.png)

混合现实 Toolkit 的所有 "默认" 配置文件均可在 SDK 项目中的文件夹 "资产/MRTK/SDK/配置文件" 中找到。

> [!IMPORTANT]
> DefaultHoloLens2ConfigurationProfile 针对 HoloLens 2 进行了优化。 有关详细信息，请参阅 [配置文件](../features/profiles/profiles.md) 。

打开主混合现实 Toolkit 配置文件时，检查器中会显示以下屏幕：
...
- [混合现实 Toolkit 配置文件配置指南](#mrtk-profile-configuration-guide)
  - [主要混合现实 Toolkit 配置文件](#the-main-mixed-reality-toolkit-configuration-profile)
...
在主混合现实 Toolkit 配置 "页上，此设置定义项目的[混合现实环境规模](/windows/mixed-reality/coordinate-systems-in-unity)的默认操作。
...
混合现实 Project 提供了一个功能强大的经过训练的输入系统，用于路由默认情况下选定的项目的所有输入事件。
...
混合现实 Project 提供了一个功能完备的 Teleportation 系统，用于管理项目中的 Teleportation 事件，默认情况下处于选中状态。
...
混合现实 Project 提供重建的空间感知系统，以便在默认情况下选择的项目中使用空间扫描系统。

<img src="../features/images/mixed-reality-toolkit-configuration-profile-screens/MRTK_SpatialAwarenessSystemSelection.png" width="650px" alt="Spatial Awareness settings 1" style="display:block;">

混合现实 Toolkit 空间感知配置使你能够定制系统的启动方式，无论应用程序是在应用程序启动时自动启动还是在以后以编程方式设置，还可以为视图字段设置范围。
...
混合现实 Toolkit 的更高级区域之一是其[服务定位器模式](https://en.wikipedia.org/wiki/Service_locator_pattern)实现，该实现允许向框架注册任何 "service"。 这样，就可以轻松地使用新功能/系统扩展框架，还可以利用这些功能来注册自己的运行时组件。
...
还可使用混合现实 Toolkit 提供的众多行组件之一在活动场景中可视化指针，或者使用自己的任何一个（如果它们实现 MRTK IMixedRealityPointer 接口）可视化指针。
...
笔势是一种特定于系统的实现，允许将输入操作分配给各种 SDK 提供的各种"笔势"输入 (例如HoloLens) 。

> [!NOTE]
> 当前的笔势实现仅适用于HoloLens，并且将针对其他系统进行增强，因为将来Toolkit这些系统 (日期) 。
...
> [!IMPORTANT]
> 系统目前仅在 Windows 10 平台（例如 HoloLens 和 Windows 10 桌面）上运行时支持语音，并且将针对其他系统进行增强，因为将来这些系统将添加到 MRTK (尚未) 。
...
Mixed Reality Toolkit的核心配置屏幕之一是能够配置和映射项目可以使用的各种类型的控制器。
...
可以通过在"配置文件"的"编辑器"下选中"使用服务检查器 *"设置* 启用服务检查器。
...
为了确保场景将所有必要的数据呈现到深度缓冲区，开发人员可以在"配置文件"的"编辑器"设置切换 *"呈现* 深度缓冲区"功能。 这会采用当前深度缓冲区，通过向主相机应用后处理效果，以颜色呈现给 [`DepthBufferRenderer`](xref:Microsoft.MixedReality.Toolkit.Rendering.DepthBufferRenderer) 场景视图。
